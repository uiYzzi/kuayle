<script lang="ts">
	...
	async function handleSubmit(e: Event) {
		...
		try {
			...
			appToast.apiError(err, i18n.t('login.error_generic'));
		} finally {
			loading = false;
		}
	}
...
<div class="flex min-h-screen items-center justify-center bg-[var(--color-bg)]">
	<div class="w-full max-w-sm space-y-6 p-8">
		<div class="text-center">
			<img src="/favicon.svg" alt={i18n.t('login.logo_alt')} class="mx-auto mb-4 h-14 w-14" />
		...
		<form onsubmit={handleSubmit} class="space-y-4">
			{#if mode === 'register'}
				<div>
					<label for="name" class="block text-sm text-[var(--color-text-secondary)]">{i18n.t('login.name')}</label>
					<Input id="name" type="text" bind:value={name} required class={authInputClass} />
				</div>
			{/if}

			<div>
				<label for="email" class="block text-sm text-[var(--color-text-secondary)]">{i18n.t('login.email')}</label>
				<Input id="email" type="email" bind:value={email} required class={authInputClass} />
			</div>

			<div>
				<label for="password" class="block text-sm text-[var(--color-text-secondary)]">{i18n.t('login.password')}</label>
			...
			>
				{#if loading}<Loader2 size={14} class="animate-spin" />{:else}{mode === 'login'
						? i18n.t('login.button.signin')
						: i18n.t('login.button.create_account')}{/if}
			</button>
		</form>
	...
	{#if demoMode && mode === 'login'}
		<div class="fixed right-4 bottom-4 z-50 flex w-[calc(100vw-2rem)] max-w-sm flex-col items-end gap-3 sm:right-6 sm:bottom-6">
			{#if demoDrawerOpen}
				<section
					aria-label={i18n.t('login.demo.aria_details')}
				...
				>
					<div class="mb-3 flex items-start justify-between gap-4">
						<div>
							<h2 class="font-medium text-[var(--color-text-primary)]">{i18n.t('login.demo.title')}</h2>
							<p class="mt-1 text-xs">{i18n.t('login.demo.description')}</p>
						</div>
						<button
							type="button"
							onclick={() => (demoDrawerOpen = false)}
							aria-label={i18n.t('login.demo.aria_close')}
							class="rounded px-2 py-1 text-xs text-[var(--color-text-tertiary)] hover:bg-[var(--color-bg)] hover:text-[var(--color-text-primary)]"
						>
							{i18n.t('login.demo.close')}
						</button>
					</div>
						...
						{#each demoUsers as user}
							...
							>
								<div class="flex items-center justify-between gap-3">
									...
									<span class="text-xs text-[var(--app-accent)]">{i18n.t('login.demo.use')}</span>
								</div>
								<dl class="mt-2 space-y-1 text-xs">
									<div class="flex justify-between gap-3">
										<dt>{i18n.t('login.email')}</dt>
										<dd class="font-mono text-[var(--color-text-primary)]">{user.email}</dd>
									</div>
									<div class="flex justify-between gap-3">
										<dt>{i18n.t('login.password')}</dt>
			...
			>
				...
				{/if}
				{i18n.t('login.demo.users_button')}
			</button>
		</div>
	{/if}
</div>
